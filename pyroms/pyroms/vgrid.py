...
class s_coordinate(object):
    ...
    def _get_Cs_r(self):
        if (self.theta_s >= 0):
            ...
            self.Cs_r = (self.c1 - self.theta_b) * \
                Ptheta + self.theta_b * Rtheta
        else:
    ...
    def _get_Cs_w(self):
        if (self.theta_s >= 0):
            ...
            Rtheta = np.tanh(self.theta_s * (self.s_w + self.p5)) / \
                (self.c2 * np.tanh(self.p5 * self.theta_s)) - self.p5
            self.Cs_w = (self.c1 - self.theta_b) * \
                Ptheta + self.theta_b * Rtheta
        else:
            self.Cs_w = self.s_w
...
class s_coordinate_4(s_coordinate):
    ...
    def __init__(self, h, theta_b, theta_s, Tcline, N, hraw=None, zeta=None):
        ...
        self.z_r = z_r(self.h,self.hc,self.N,self.s_rho,
            self.Cs_r,self.zeta,self.Vtrans)
        self.z_w = z_w(
            self.h,
            self.hc,
            self.Np,
        ...
        if self.Vtrans == 1:
            for n in range(ti):
                for k in range(self.N):
                    z0 = self.hc * self.s_rho[k] + \
                        (self.h - self.hc) * self.Cs_r[k]
        ...
        if self.Vtrans == 1:
            for n in range(ti):
                for k in range(self.Np):
                    z0 = self.hc * self.s_w[k] + \
                        (self.h - self.hc) * self.Cs_w[k]
